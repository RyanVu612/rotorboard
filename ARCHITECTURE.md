...
| `DashboardLayoutModel` | **Implemented:** Widget layout model; persists widget descriptors to QSettings |
...
| `voltageHistory` | list of double | Recent voltage samples |
| `pwmHistory` | list of double | Recent PWM samples |
...
**Today:** `QGuiApplication` + `QQmlApplicationEngine`, `AppController` wires `TelemetryManager` + `MotorTelemetryModel` + `DashboardLayoutModel`, loads `qml/Main.qml`. CLI flags: `--log [path]` (record session CSV; default `logs/session-YYYYMMDD-HHMMSS.csv`, bare filenames land in `logs/`), `--playback path` (replay CSV instead of fake source), `--mavlink [host:port]` (UDP MAVLink input; default `0.0.0.0:14550`). `--playback` and `--mavlink` are mutually exclusive (MAVLink wins if both are passed). `AppController.sourceLabel` drives the dashboard source badge. `AppController.chartsFrozen` and `AppController.editMode` drive global chart pause and layout editing.
...
| `DashboardPage.qml` | Page chrome, Pause/Resume, Edit layout toggle, grid + palette |
| `DashboardGrid.qml` | 16×20 coordinate grid (88×72 px cells), widget placement engine |
| `WidgetPalette.qml` | Edit-mode panel to add/configure/delete widgets |
| `widgets/ValueTile.qml` | 1×1 scalar tile for one motor metric |
| `widgets/ChartTile.qml` | Large chart tile wrapping `Sparkline` for one metric |
| `widgets/MotorSummaryCard.qml` | Multi-cell all-in-one motor summary |
| `widgets/DashboardWidgetSlot.qml` | Draggable/resizable grid slot wrapper |
| `StatusBadge.qml` | Warning/stale visual (color, label) |
| `Sparkline.qml` | Canvas-based time-series chart; supports global freeze snapshot |

**Implemented:** Configurable widget dashboard. Each widget binds to a motor + metric (or full summary). Layout persists in QSettings (`Dashboard/widgetsJson`). Default chart widgets span 4×3 cells for readability. Edit mode supports drag, resize, add, and delete. Global Pause freezes all sparklines for analysis; Resume jumps back to live data. Legacy `MotorGrid/positionsJson` migrates to `motorSummary` widgets on first load.
...
└── qml/
    ...
    └── widgets/
        ├── ValueTile.qml
        ├── ChartTile.qml
        ├── MotorSummaryCard.qml
        └── DashboardWidgetSlot.qml
```
...
| `DashboardLayoutModel` + configurable widget grid | Done |
| `Sparkline.qml` with global freeze support | Done |
| Widget dashboard (`ValueTile`, `ChartTile`, `MotorSummaryCard`) | Done |
...
9. **Advanced widgets** (charts, layouts) — configurable widget dashboard with edit mode, large chart tiles, and global chart pause done
